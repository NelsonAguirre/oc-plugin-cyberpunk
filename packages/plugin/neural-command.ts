...
// Filter out empty lines (lines with only Braille blanks/spaces) and convert to code points
const cloudRows = NEURAL_BRAIN
  .filter((line) => {
    // Check if line has any non-blank characters
    for (const char of line) {
      const cp = char.codePointAt(0) ?? 32
      if (cp !== BRAILLE_BLANK && !/\s/u.test(char)) {
        return true
      }
    }
    return false
  })
  .map((line) => Array.from(line, (char) => char.codePointAt(0) ?? 32))
...
const createNeuralPostProcess = (onDone: () => void) => {
  let elapsed = 0
  let finished = false
  ...
  const ensureCloudLayout = (width: number, height: number) => {
    ...
    const startY = Math.max(0, Math.floor((height - cloudRows.length) * 0.45))
  ...
  const drawCloud = (buf: any) => {
    ...
    for (let i = 0; i < cloudGlyphs.length; i++) {
      ...
      // Slower, more visible pulse for neural effect
      const pulse = Math.sin(time * 3.0 + glyph.phase * 2.0)
      const driftMotion = Math.sin(time * 1.5 + glyph.drift * 3.0 + glyph.phase)
      
      // Combine motions for more visible "breathing" effect
      const motion = (pulse * 0.6 + driftMotion * 0.4)

      // Higher base density for more visible dots
      const density = clamp01(glyph.baseDensity * 0.85 + 0.25 + motion * 0.35)
      ...
      // Stronger flicker for visibility
      const flicker = Math.abs(motion) * 0.25

      // Brighter base colors with enhanced contrast
      fg[colorIndex] = clamp01(glyph.baseR * 1.3 + flicker * 0.2)
      fg[colorIndex + 1] = clamp01(glyph.baseG * 1.2 + flicker * 0.4)
      fg[colorIndex + 2] = clamp01(glyph.baseB * 1.3 + flicker * 0.15)
      fg[colorIndex + 3] = 1
    }
  }
...
export const createNeuralCommand = (api: Api) => {
  ...
  const run = () => {
    stop()
    postProcess = createNeuralPostProcess(stop)
