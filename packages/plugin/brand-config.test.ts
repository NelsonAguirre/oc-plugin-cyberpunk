...
// ─── Task 4.1 — Local asset wins over global (Two-Tier-Asset-Resolution) ────

describe("4.1 — Local asset wins over global (Two-Tier-Asset-Resolution)", () => {
...
// ─── Task 4.2 — Global asset used when no local override ────────────────────

describe("4.2 — Global asset used when no local override exists", () => {
...
// ─── Task 4.3 — Built-in default when neither local nor global asset exists ─

describe("4.3 — Built-in default when neither local nor global asset exists", () => {
...
// ─── Task 4.4 — localDir omitted → global-only resolution ───────────────────

describe("4.4 — localDir omitted → global-only resolution (Optional-LocalDir-Parameter)", () => {
...
// ─── Task 4.5 — local brand.json name overrides global (Brand-Name-Override) ─

describe("4.5 — localDir/brand.json name overrides globalDir/brand.json name (Brand-Name-Override)", () => {
  ...
  it("brand name is capped at 20 characters", () => {
    writeBrandJson(localDir, { name: "A_VERY_LONG_BRAND_NAME_THAT_EXCEEDS_LIMIT" })
...
// ─── Task 4.6 — Per-asset independence (Per-Asset-Independence) ─────────────

describe("4.6 — Per-asset independence: one local override does not affect others", () => {
...
// ─── Task 4.7 — Integration: correct dirs derived from api.state.path ────────

describe("4.7 — Integration: tui() derives correct globalDir/localDir from api.state.path", () => {
  /**
  ...
  it("globalDir is derived as join(config, 'oc-neo-terminal')", () => {
    ...
    expect(expectedGlobalDir).toBe("/home/user/.config/opencode/oc-neo-terminal")
  })
  ...
  it("localDir is derived as join(directory, '.opencode', 'oc-neo-terminal')", () => {
    ...
    expect(expectedLocalDir).toBe("/home/user/myproject/.opencode/oc-neo-terminal")
  })
