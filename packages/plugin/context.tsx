...
const logContext = (...args: unknown[]) => {
  console.log("[NEXUS][CTX]", ...args)
}

export const NexusContext = (props: { theme: TuiThemeCurrent; api: Api; sessionId: string; brandName?: string }) => {
    ...
    <Show when={data().hasData}>
      ...
      >
        <text fg={props.theme.primary}>
          <b>{props.brandName || "NEXUS"}</b>
        </text>
        <Show when={data().contextLimit > 0}>
          <text>
            <span style={{ fg: props.theme.textMuted }}>MEM </span>
            <span style={{ fg: ctxColor() }}>
              [{bar(data().contextRatio, fillWidth(barWidth(), "MEM ", ctxPct()))}]
            </span>
        ...
        <Show when={data().outputLimit > 0}>
          <text>
            <span style={{ fg: props.theme.textMuted }}>GEN </span>
            <span style={{ fg: props.theme.primary }}>
              [{bar(data().outputRatio, fillWidth(barWidth(), "GEN ", outPct()))}]
            </span>
        ...
        <text>
          <span style={{ fg: props.theme.textMuted }}>HIT </span>
          <span style={{ fg: props.theme.success }}>
            [{bar(data().cacheRatio, fillWidth(barWidth(), "HIT ", cachePct()))}]
          </span>
